...
\begin{theorem}(\cite{htt} 5.5.4.15)
    Let $C$ be a presentable $\infty-$category, $S$ be a set of morphisms in $C$. Let $C'$ be the full subcategory of $C$ spanned by S-local objects, then the inclusion $C' \hookrightarrow C$ admits a left adjoint $L_S:C\to C'$.
\end{theorem}
...
\begin{definition}
    Let $L_{\text{mot}}:\text{PSh}(Sm/S) \to \text{PSh}(Sm/S)$ be the S-localization functor with respect to the set $S$ defined above. We call $L_{\text{mot}}$ the motivic localization functor.
\end{definition}
