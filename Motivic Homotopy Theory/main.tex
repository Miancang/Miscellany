...
\subsection{Closed symmetric monoidal structure on $\mathcal{H}(S)_\bullet$}
...
The classic stabilization process depend ont the suspension functor defined by $\infty$-pushout against the base point. In $\operatorname{Sp}$, that corresponds to the smash product with $S^1$. The generalization we make here is to replace $S^1$ by an arbitrary object $X$ in a closed presentable symmetric monoidal pointed $\infty$-category. By the closed structure, we can define the loop functor in duality.

\begin{definition}
    Let $\mathcal{C}^\otimes$ be a closed presentable symmetric monoidal pointed $\infty$-category, $X\in \mathcal{C}$ be an object. The suspension functor $\Sigma_X:\mathcal{C} \to \mathcal{C}$ is defined as the functor: $-\otimes X$, while the loop functor $\Omega_X:\mathcal{C} \to \mathcal{C}$ is defined as the functor: $\mathcal{H}om(X,-)$.
\end{definition}

\begin{definition}
    Let $\mathcal{C}^\otimes$ be a closed presentable symmetric monoidal pointed $\infty$-category, $X\in \mathcal{C}$ be an object. The stable $\infty$-category $\mathcal{C}[X^{-1}]$ is defined as:

    $ \operatorname{lim}[\mathcal{C} \xleftarrow{\Omega_X} \mathcal{C} \xleftarrow{\Omega_X} \cdots] $
\end{definition}

We also have the universal property of such stabilization.

\begin{theorem}[Robalo]\label{thm:robalo's stabilization}
    Let $\mathcal{C}^\otimes$ be a closed presentable symmetric monoidal pointed $\infty$-category, $X\in \mathcal{C}$ be an object. For every $\mathcal{D}^\otimes \in \operatorname{Pr}^{\otimes,L}_{\text{St}}$ and morphism $F: \mathcal{C}^{\otimes} \rightarrow \mathcal{D}^{\otimes}$ in $\operatorname{Pr}^{\otimes,L}$ for which the object $F(X)$ is invertible (admits an inverse with respect to $\otimes$), there exists an essentially unique morphism $\hat{F}: \mathcal{C}[X^{-1}]^{\otimes} \rightarrow \mathcal{D}^{\otimes}$ in $\operatorname{Pr}^{\otimes,L}$ such that the following diagram commutes:
